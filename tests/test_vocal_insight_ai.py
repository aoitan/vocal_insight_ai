...
from vocal_insight_ai import (
    analyze_audio_segments,
    analyze_segment_with_praat,
...
def test_process_boundaries():
    ...
    final_boundaries = process_boundaries(
        boundaries_sec, total_duration, min_len, max_len
    )
...
def test_analyze_audio_segments():
    ...
    results, prompt = analyze_audio_segments(
        y, sr, filename, config=default_analysis_config
    )
